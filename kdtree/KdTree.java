...
public class KdTree {
    ...
    // NOTE: profile why 600K input so slow....
    ...
    /*
      ...
      - Recursively search left/bottom (if any could fall in
      rectangle).
      - Recursively search right/top (if any could fall in
      rectangle).
    */
    ...
    private Point2D findNearest2(Node node, Point2D p,
                                 double distance,
                                 boolean  vertical)
    {
        if (node == null)
        ...
        if (DEBUG)    StdOut.printf("Find N: P: %s\n", node.p);


        Point2D p2 = null;
        double nodeToP = p.distanceSquaredTo(node.p);
    
        if ((vertical && p.x() < node.p.x())
            || (!vertical && p.y() < node.p.y())) {
            p2 = findNearest2(node.lb, p, nodeToP, !vertical);
            if (p2 == null
                || p2.distanceSquaredTo(p) >= nodeToP
                //          || node == root
                //          || needsWalkSides(nodeToP, node.rt, p)
                )
                p2 = findNearest2(node.rt, p, nodeToP, !vertical);
        } else if ((vertical && p.x() >= node.p.x())
                   || (!vertical && p.y() >= node.p.y())) {
            p2 = findNearest2(node.rt, p, nodeToP, !vertical);
            if (p2 == null
                || p2.distanceSquaredTo(p) > nodeToP
                //          || node == root
                //          || needsWalkSides(nodeToP, node.lb, p)
                )
                p2 = findNearest2(node.lb, p, nodeToP, !vertical);
        }

        if (p2 == null || nodeToP < p.distanceSquaredTo(p2))
            return node.p;
        else
            return p2;
    }
